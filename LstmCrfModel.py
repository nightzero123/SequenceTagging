...
class BiLSTM_CRF(nn.Module):
    ...
    def __init__(self, parameter):
        ...
        self.lstm = nn.LSTM(self.embedding_dim, self.hidden_dim // 2, num_layers=3, bidirectional=True)
    ...
    def _forward_alg(self, feats):
        ...
        for feat in feats:
            ...
            forward_var = torch.stack(alphas_t).squeeze(-1).view(1, -1)
    ...
    def _viterbi_decode(self, feats):
        ...
        for feat in feats:
            ...
            for next_tag in range(self.tagset_size+2):
                ...
                # print('next_tag_var:'+str(next_tag_var))
                best_tag_id = argmax(next_tag_var)
                bptrs_t.append(best_tag_id)
                viterbivars_t.append(next_tag_var[0][best_tag_id])
                # print('viterbivars_t:'+str(viterbivars_t))
            # Now add in the emission scores, and assign forward_var to the set
            # of viterbi variables we just computed
            # print(viterbivars_t)
            forward_var = (torch.stack(viterbivars_t).squeeze(-1) + feat).view(1, -1)
